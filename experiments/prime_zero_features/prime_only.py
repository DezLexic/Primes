"""Prime-only finite signals and feature detection; no zero-loader dependency.

Extracted without numerical changes from model.py for independent blind runs.
The ordinary Euler product does not naively converge on the critical line.
"""
...
def detect_extrema(tau, values, signal, prominence=0.1, min_width=0.0, kinds=("peak", "min")):
    ...
    rows = []
    if not kinds or any(kind not in ("peak", "min") for kind in kinds):
        raise ValueError("kinds must contain peak and/or min")
